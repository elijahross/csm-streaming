...
function showVoiceCircle() {
  const c =
    document.getElementById("voice-circle") || createPermanentVoiceCircle();
...
function hideVoiceCircle() {
  ...
  if (c) {
    ...
    c.style.setProperty("--dynamic-scale", 1);
  }
}
...
function connectWebSocket() {
  if (reconnecting && reconnectAttempts >= maxReconnectAttempts) {
    console.error(
      "Maximum reconnect attempts reached. Please refresh the page."
    );
  ...
  ws.onopen = () => {
    ...
    if (!reconnecting) {
      addMessageToConversation(
        "ai",
        "WebSocket connected. Ready for voice or text."
      );
    } else {
...
function requestInterrupt() {
  ...
  if (ws && ws.readyState === WebSocket.OPEN) {
    ...
    setTimeout(() => {
      if (interruptInProgress) {
        console.log(
          "No interrupt confirmation received from server, resetting state"
        );
        interruptInProgress = false;
      }
    }, 2000);
  ...
  switch (d.type) {
      ...
      try {
        if (audioContext) {
          if (audioContext.state === "suspended") {
            audioContext
              .resume()
              .catch((e) => console.warn("Error resuming audio context:", e));
          }
        } else {
    ...
    case "audio_chunk":
      ...
      if (d.gen_id === activeGenId || activeGenId === 0) {
        ...
        console.log(
          `Ignored stale chunk - current gen: ${activeGenId}, received: ${d.gen_id}`
        );
      }
      break;
    ...
    case "audio_status":
      ...
      if (d.status === "generating") {
        console.log(
          "GOT GENERATING STATUS - IMMEDIATELY CLEARING ALL INTERRUPT FLAGS"
        );
        interruptRequested = false;
        interruptInProgress = false;
        ...
        d.status === "interrupt_acknowledged"
      ) {
    ...
    case "vad_status":
      if (d.status === "speech_started") {
        console.log(
          `[VAD] speech_started | should_interrupt=${d.should_interrupt}`
        );
...
function queueAudioForPlayback(arr, sr, genId = 0) {
  if (activeGenId !== 0 && genId !== activeGenId) {
    console.log(
      `Stale chunk ignored (genId mismatch): ${genId} vs ${activeGenId}`
    );
    return;
  }
...
function processAudioPlaybackQueue() {
  ...
  if (activeGenId !== 0 && genId !== activeGenId) {
    console.log(
      `Skipping stale chunk playback (gen ${genId} vs active ${activeGenId})`
    );
...
async function setupChatUI() {
  ...
  interruptBtn.className =
    "px-3 py-2 ml-2 bg-red-600 text-white rounded hover:bg-red-700 flex items-center transition duration-150";
  interruptBtn.innerHTML =
    '<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8 7a1 1 0 00-1 1v4a1 1 0 001 1h4a1 1 0 001-1V8a1 1 0 00-1-1H8z" clip-rule="evenodd" /></svg> Stop';
  interruptBtn.onclick = (e) => {
    ...
    try {
      ...
      setTimeout(
        () => interruptBtn.classList.remove("bg-red-800", "scale-95"),
        150
      );
    } catch (error) {
